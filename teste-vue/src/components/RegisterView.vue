...
const submitForm = async () => {
  ...
  const phoneUnmasked = unmaskPhone(phone.value) // Remove a máscara antes de salvar

  try {
    const newUser = await userStore.addUser({
      name: name.value,
      email: email.value,
      phone: phoneUnmasked, // Salva o telefone sem máscara
    })
...
<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-100 px-4">
    <div class="w-full max-w-md bg-white p-6 rounded-2xl shadow-lg">
      <h2 class="text-2xl font-bold text-center text-gray-800 mb-6">Cadastro de Usuário</h2>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="flex justify-center gap-4 w-full">
            <BaseButton label="Salvar" bgColor="bg-blue-600" textColor="text-white" type="submit" class="rounded hover:bg-blue-700 w-full py-2"/>
            <BaseButton label="Cancelar" bgColor="bg-red-500" textColor="text-white" @click="goToHome" class="rounded hover:bg-red-600 w-full py-2"/>
        </div>
      </form>
    </div>
  </div>
